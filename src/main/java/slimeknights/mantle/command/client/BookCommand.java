...
public class BookCommand {
  ...
  private static int doExportImages(ResourceLocation book, int scale) {
    ...
    if(bookData != null) {
      if(!screenshotDir.toFile().mkdirs() && !screenshotDir.toFile().exists()) {
        throw new CommandRuntimeException(new TranslatableComponent(EXPORT_FAIL_IO));
      }
      ...
      bookData.load();
      BookScreen screen = new BookScreen(new TextComponent("Book"), bookData, "", null, null);
          ...
          try (NativeImage image = takeScreenshot(target)) {
            ...
            if (page == -1) { // the cover is half the width
              try (NativeImage scaled = new NativeImage(image.format(), width / 2, height, false)) {
                ...
                throw new CommandRuntimeException(new TranslatableComponent(EXPORT_FAIL));
              }
            } else {
    ...
    if (player != null) {
      Component fileComponent = new TextComponent(screenshotDir.toString()).withStyle(ChatFormatting.UNDERLINE)
        .withStyle(style -> style.withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_FILE, screenshotDir.toAbsolutePath().toString())));
      player.displayClientMessage(new TranslatableComponent(EXPORT_SUCCESS, fileComponent), false);
    }
    return 0;
  }
  ...
  public static void bookNotFound(ResourceLocation book) {
    Player player = Minecraft.getInstance().player;
    if (player != null) {
      player.displayClientMessage(new TranslatableComponent(BOOK_NOT_FOUND, book).withStyle(ChatFormatting.RED), false);
    }
  }
}
